...
#change for conditional GAN
class TensorVMSplit_GAN(TensorBase):
    def __init__(self, aabb, gridSize, device, **kargs):
        super(TensorVMSplit_GAN, self).__init__(aabb, gridSize, device, **kargs)
        #for make latent vector, hyperparameter
        self.z_dim = 128

    # init_svd_volume: initialize the svd volume
    def init_svd_volume(self, res, device):
        self.density_plane, self.density_line = self.init_one_svd(self.density_n_comp, self.gridSize, 0.1, device)
        self.app_plane, self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, 0.1, device)
        self.basis_mat = torch.nn.Linear(sum(self.app_n_comp), self.app_dim, bias=False).to(device)
    
    # init shape code z_s and appearance code z_a from gaussian, extract some latent vectors for appearance and shape
    def init_latent(self, batch_size, device):
        self.z_s = torch.randn(batch_size, self.z_dim).to(device) 
        self.z_a = torch.randn(batch_size, self.z_dim).to(device) #appearance code
    
    # init_one_svd: initialize one svd volume
    ...
    #update get_optparam_groups for conditional GAN (including z_s and z_a)
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [{'params': self.density_line, 'lr': lr_init_spatialxyz}, {'params': self.density_plane, 'lr': lr_init_spatialxyz},
                     ...
                     {'params': self.basis_mat.parameters(), 'lr':lr_init_network}, 
                     {'params': self.z_s, 'lr':lr_init_network}, 
                     {'params': self.z_a, 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        return grad_vars
