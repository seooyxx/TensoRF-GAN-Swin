...
def reconstruction(args):
    ...
    discriminator = SwinTransformerV2().to(device)
    ...
    for iteration in pbar:
        ...
        rgb_map_fake, alphas_map_fake, depth_map_fake, weights_fake, uncertainty_fake = renderer(rays_train, tensorf, chunk=args.batch_size, N_samples=nSamples, white_bg = white_bg, ndc_ray=ndc_ray, device=device, is_train=True)
        ...
        if not (iteration % 5):
            ...
            summary_writer.add_scalar('train/total_loss', total_loss, global_step=iteration)
        ...
        if iteration % args.progress_refresh_rate == 0:
            pbar.set_description(
                ...
                + f' mse = {loss:.6f}'
                + f' D_loss = {discriminator_loss:.3f}'
                + f' G_loss= {generator_loss:.3f}'
